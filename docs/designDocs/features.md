...
- **オプション**:
    ...
    - `--exclude-shorts` (`-e`): ショート動画（YouTube Shorts）を除外して表示します。
        ...
        - **動作**: 
            ...
            - Gemini API (`gemini-3-flash-preview`) を使用して以下の構成で要約します。
                ...
                2.  **見どころ**: 特に盛り上がったシーンや面白い発言（3点程度） **(タイムスタンプ付き [MM:SS] または [HH:MM:SS])**
                3.  **セットリスト**: 歌枠の場合、歌われた曲名リスト **(タイムスタンプ付き [MM:SS] または [HH:MM:SS])**
                4.  **告知・重要事項**: 配信内で触れられた予定や告知
            - 要約結果をリストの上部に表示します。
        - **キャッシュ機能 (New)**:
            - **取得した字幕データはローカルにキャッシュされます。（クリエイターごとに最大50件まで保持し、古くなったものは破棄されます）**
            - **同一動画に対して2回目以降の要約実行時は、APIへのリクエストをスキップしてキャッシュを利用するため高速なレスポンスが可能です。**
...
### 8. `export`
`check --summary` コマンドでキャッシュされた動画の字幕データを指定フォーマットでエクスポートします。
- **機能**:
    - キャッシュディレクトリ (`~/.config/valiv/transcripts-cache/`) 内のインデックス情報に基づき、対象クリエイターの字幕データをファイル出力します。
    - **動画ごとに1ファイル** 出力されます。
    - ファイル名規則: `[creatorId]_[videoId].[拡張子]`
    - 出力フォーマット: 
        - `json`: 生のキャッシュデータ構造
        - `text`: タイムスタンプを含まない単なる文章
        - `md` (デフォルト): タイムスタンプを含み、可読性を高めたMarkdown形式
- **オプション**:
    - `-o, --output <dir>`: 出力先ディレクトリを指定します。（デフォルト: カレントディレクトリ下の `valiv-exports` フォルダなど）
    - `--id <creatorId>`: 指定したクリエイターのキャッシュのみをエクスポート対象とします。
    - `--format <format>`: 出力形式を指定します (`json`, `text`, `md`)。
- **使用法**:
    - `valiv export`
    - `valiv export --id manaka_tomori --format md -o ./my-exports`
...
### エラーハンドリング
- 外部サービス（YouTube, Twitch, Google Calendar等）からのデータ取得に失敗した場合、詳細な技術的エラー（スタックトレース等）は表示せず、ユーザーフレンドリーなメッセージ「時間を置いて再度お試し下さい。」を表示します。
- 一部のクリエイターのデータ取得に失敗しても、他のクリエイターのデータが表示可能な場合は、取得できたデータのみを表示し、失敗した箇所についてはエラーメッセージを表示して処理を継続します。
