...
### 1. `init`
初期設定を行います。
- **機能**: ユーザーにアスキーアートによるウェルカムメッセージを表示し、vα-liv公式メンバー（灯里愛夏、上水流宇宙、レトラ、公式）の一括登録を提案します。その後、任意のクリエイター登録（`add`コマンド相当）を促します。
- **使用法**: `valiv init`
...
- **機能**:
    ...
    - **表示内容**:
        - イベントのタイトルと時間のみを表示するシンプルなリスト形式です。
        - URLや詳細な説明文は表示されません。
    - **ページネーション**:
        - 1ページあたり10件を表示します。
        - `←` `→` キーでページ切り替えが可能です。
...
- **将来展望**: 次期開発にて YouTube Data API と連携し、より多くのデータ取得と正確な配信予定の表示を計画しています。
