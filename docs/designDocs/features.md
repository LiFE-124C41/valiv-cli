...
- **用途**: `list` コマンドにおける YouTube および X (Twitter) の登録者数、再生数、動画数、フォロワー数、ポスト数等の統計情報と前日比の表示
- **仕組み**: `stats_bot` が更新する公開スプレッドシートの `daily_stats` シート（YouTube統計用）および `x_stats` シート（X統計用）から、CSVエクスポート機能を利用してそれぞれデータを取得・マージします。
