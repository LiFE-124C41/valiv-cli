...
export class TwitchService {
    ...
    async getChannelInfo(username: string): Promise<TwitchUser | null> {
        try {
            const data = await this.makeRequest<{ data: TwitchUser[] }>('https://api.twitch.tv/helix/users', {
                login: username,
            });
    ...
    async getLiveStreams(creators: Creator[]): Promise<Activity[]> {
        ...
        try {
            ...
            for (const chunk of chunks) {
                ...
                const data = await this.makeRequest<{ data: TwitchStream[] }>('https://api.twitch.tv/helix/streams', {
                    user_login: chunk,
                });
            ...
            return streams.map(stream => {
                const creator = creators.find(
                    c => c.twitchChannelId?.toLowerCase() === stream.user_login.toLowerCase() ||
                ...
                return {
                    ...
                    description: stream.game_name ? `Playing ${stream.game_name}` : 'Live Stream',
                    author: creator,
    ...
    async getUpcomingSchedules(creators: Creator[]): Promise<ScheduleEvent[]> {
        ...
        for (const creator of creatorsWithTwitch) {
            try {
                ...
                const data = await this.makeRequest<{ data: { segments: TwitchScheduleSegment[] }, pagination: TwitchPagination }>(
                    ...
                    segments.forEach(segment => {
                        if (!segment.canceled_until) {
                            events.push({
                                ...
                                author: creator,
                                status: 'upcoming',
                                description: segment.category ? `Category: ${segment.category.name}` : undefined
                            });
                        }
                    });
                }
    ...
    async getRecentVideos(creators: Creator[]): Promise<Activity[]> {
        ...
        for (const creator of creatorsWithTwitch) {
            try {
                ...
                const data = await this.makeRequest<{ data: TwitchVideo[] }>('https://api.twitch.tv/helix/videos', {
                    user_id: userInfo.id,
