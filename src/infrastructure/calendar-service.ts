...
import { Creator, ScheduleEvent } from '../domain/models';
import { IScheduleService } from '../domain/interfaces';
...
export class CalendarService implements IScheduleService {
    async getSchedules(creators: Creator[]): Promise<ScheduleEvent[]> {
            ...
            .map(async (creator) => {
                try {
                    ...
                    return Object.values(events)
                        ...
                        .filter((event) => event.startTime >= now); // Future events only
                } catch (error) {
                    console.error(
        ...
        return results.flat().sort((a, b) => a.startTime.getTime() - b.startTime.getTime());
    }
}
