        ...
        try {
          ...
          for (let i = lines.length - 2; i >= 1; i--) {
            ...
            if (row[0] === yesterdayStr) {
              ...
              if (row.length >= 3)
                yesterdaysVideoCount = parseInt(row[2].replace(/,/g, ''), 10);
              if (row.length >= 4)
                yesterdaysViewCount = parseInt(row[3].replace(/,/g, ''), 10);
              foundYesterday = true;
              break;
            }
          }
          ...
          if (!foundYesterday && lines.length >= 3) {
            ...
            if (prevData.length >= 2) {
              yesterdaysSubscriberCount = parseInt(
                prevData[1].replace(/,/g, ''),
                10,
              );
              if (prevData.length >= 3)
                yesterdaysVideoCount = parseInt(
                  prevData[2].replace(/,/g, ''),
                  10,
                );
              if (prevData.length >= 4)
                yesterdaysViewCount = parseInt(
                  prevData[3].replace(/,/g, ''),
                  10,
                );
            }
          }
