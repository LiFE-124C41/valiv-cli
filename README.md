...
*   **エラーハンドリングとログ**:
    *   外部サービスとの通信エラーが発生した場合、詳細な技術情報は表示されず「時間を置いて再度お試し下さい。」という簡潔なメッセージが表示されます。
    *   詳細なスタックトレースや、`--debug` (`-d`) オプションによる MPV の詳細ログは、設定ディレクトリ（`~/.config/valiv-cli-nodejs/valiv_debug.log` など）に出力されます。
    *   トラブルシューティングの際は、このログファイルを確認してください。
*   **その他**:
